...
impl Context {
    fn run(&self, mut core: Box<Core>) -> RunResult {
        while !core.is_shutdown {
            ...
            if let Some(task) = core.next_task(&self.worker) {
                info!("[tokio] [NextWork] [src/runtime/thread_pool/worker.rs:323] [info] $ run() >> {}: {:?}", "Get next task", task);
            ...
            if let Some(task) = core.steal_work(&self.worker) {
                ...
                // Wait for work
                info!("[tokio] [WaitWork] [src/runtime/thread_pool/worker.rs:335] [info] $ run() >> {}", "Wait for task");
                core = self.park(core);
            }
        }
    ...
    fn run_task(&self, task: Notified, mut core: Box<Core>) -> RunResult {
        ...
        *self.core.borrow_mut() = Some(core);
        ...
        // Run the task
        coop::budget(|| {
            info!("[tokio] [RunWork] [src/runtime/thread_pool/worker.rs:355] [info] $ run_task() >> {}: {:?}", "Running given task", task);
            task.run();
            ...
            loop {
                ...
                if coop::has_budget_remaining() {
                    ...
                    info!("[tokio] [LIFORun] [src/runtime/thread_pool/worker.rs:368] [info] $ run_task() >> {}: {:?}", "Running LIFO because budget remains", task);
                    *self.core.borrow_mut() = Some(core);
                    task.run();
                } else {
                    // Not enough budget left to run the LIFO task, push it to
                    // the back of the queue and return.
                    info!("[tokio] [LIFONoMore] [src/runtime/thread_pool/worker.rs:375] [info] $ run_task() >> {}: {:?}", "Pushing back LIFO because no more budget", task);
...
impl Core {
    ...
    fn next_task(&mut self, worker: &Worker) -> Option<Notified> {
        if self.tick % GLOBAL_POLL_INTERVAL == 0 {
            worker.inject().pop().map_or_else(|| self.next_local_task(), |task| {
              info!("[tokio] [GetFromGlobal] [src/runtime/thread_pool/worker.rs:459] [info] $ next_task() >> {}: {:?}", "Get task from global because of GLOBAL_POLL_INTERVAL", task);
              Some(task)
            })
        } else {
            self.next_local_task().or_else(|| {
              let task = worker.inject().pop();
              info!("[tokio] [GetFromGlobal] [src/runtime/thread_pool/worker.rs:465] [info] $ next_task() >> {}: {:?}", "Get task from global because no task in local queue", task);
              task
            })
        }
    }
    ...
    fn next_local_task(&mut self) -> Option<Notified> {
        self.lifo_slot.take().map_or_else(|| {
          ...
          info!("[tokio] [GetFromLocal] [src/runtime/thread_pool/worker.rs:474] [info] $ next_local_task() >> {}: {:?}", "Get task from local queue because no task in lifo_slot", task);
          task
        }, |task| {
          info!("[tokio] [GetFromLIFO] [src/runtime/thread_pool/worker.rs:477] [info] $ next_local_task() >> {}: {:?}", "Get task from lifo_slot because it exists", task);
          Some(task)
        })
    }
...
impl Shared {
    pub(super) fn schedule(&self, task: Notified, is_yield: bool) {
        CURRENT.with(|maybe_cx| {
            ...
            info!("[tokio] [ScheduleGlobal] [src/runtime/thread_pool/worker.rs:756] [info] $ schedule() >> {}: {:?}", "Schedule to global because task is not part of current thread", task);
    ...
    fn schedule_local(&self, core: &mut Core, task: Notified, is_yield: bool) {
        ...
        let should_notify = if is_yield {
            info!("[tokio] [ScheduleQueue] [src/runtime/thread_pool/worker.rs:720] [info] $ schedule_local() >> {}: {:?}", "Scheduling to queue because it should yield", task);
            ...
            info!("[tokio] [ScheduleLIFO] [src/runtime/thread_pool/worker.rs:732] [info] $ schedule_local() >> {}: {:?}", "Scheduling to LIFO because it should not yield", task);
